...
if __name__ == '__main__':
  ...
  parser.add_argument('--nony_dir', type=str)
  ...
  if os.path.isdir(args.nony_dir):
    shutil.rmtree(args.nony_dir)
  os.makedirs(args.nony_dir)
  hit_mp3_dir = os.path.join(args.nony_dir, 'mp3s')
  ...
  for method_tag, fns in method_tag_to_fns.items():
    for fn in fns:
      ...
      out_fp = os.path.join(hit_mp3_dir, '{}.wav'.format(wav_uuid))
      with tempfile.NamedTemporaryFile() as f:
        shutil.copyfile(wav_fp, f.name)
        cmd = 'ffmpeg -i {} -ss {} -t {} -loglevel error -hide_banner -y {}'.format(f.name, offset, args.slice_len, out_fp)
        res = subprocess.call(cmd, shell=True)
        if res > 0:
          raise Exception('ffmpeg failed')

  # Create key
  with open(os.path.join(args.nony_dir, 'key.csv'), 'w') as f:
